...
sub constructIsmFile
{
	...
	# Get the APP name from check $CONFIG_TYPE
	my $appName = "NLSPOLEventHandlerApp.app"; # default app name
	my $config_type = $ENV{CONFIG_TYPE};
	if ("$config_type" eq "pcv_smdc")
	{
		$appName = "NLSPOLEventHandlerApp_PCV.app";
	}
	# Read ISM file
	my	$data = "";
...
#		$buf =~ s#<row><td>ProductName</td><td>[^<]*</td><td/></row>#<row><td>ProductName</td><td>Nextlabs Common Libraries</td><td/></row>#g;
		...
		$buf =~ s#NLSPOLEventHandlerApp.app#$appName#g;
		$data .= $buf;
	}
